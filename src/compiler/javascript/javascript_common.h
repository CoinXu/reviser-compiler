...
namespace javascript {
  extern std::map<DataType, string> JavaScriptBuildInDataTypeMap;
  extern std::map<DataType, vector<ReviserType>> JavaScriptDataTypeReviserMap;
  ...
  public:
    ...
    const static vector<ReviserSyntaxDefinition> FindReviserSyntaxDefinitionByDataType(DataType type) {
      ...
      if (JavaScriptDataTypeReviserMap.find(type) == JavaScriptDataTypeReviserMap.end()) {
        return defs;
      }

      for (ReviserType t : JavaScriptDataTypeReviserMap.at(type)) {
